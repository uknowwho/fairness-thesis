...
taiwan_df = pd.read_csv("taiwan.csv", sep=';', header=1)
taiwan_df.drop("ID", inplace=True, axis=1)
pre_value_counts = taiwan_df["default payment next month"].value_counts(normalize=True)
print(pre_value_counts)

print(taiwan_df.isnull().sum().sum())
categorical_names = ["SEX", "EDUCATION", "MARRIAGE", 'PAY_0', 'PAY_2', 'PAY_3', 'PAY_4', 'PAY_5', 'PAY_6']

categorical_ixs = [taiwan_df.columns.get_loc(name) for name in categorical_names]

mappings = {}
for c in taiwan_df.columns:
    if c in categorical_names:
        if not all([x.isnumeric() for x in taiwan_df[c].values if type(x) == str]):
            categories = set(taiwan_df[c].values)
            if np.nan in categories:
                categories.remove(np.nan)
            mapping = dict(zip(categories, range(len(categories))))
            mappings[c] = mapping
            taiwan_df[c] = taiwan_df[c].map(mapping)
    taiwan_df[c] = taiwan_df[c].astype('float64')


# drop labels from input
labels = taiwan_df["default payment next month"]
taiwan_df.drop("default payment next month", inplace=True, axis=1)

# generate more examples of positive label
oversample = imblearn.over_sampling.SMOTENC(sampling_strategy= 2*pre_value_counts[1] / pre_value_counts[0], categorical_features=categorical_ixs)
X, y = oversample.fit_resample(taiwan_df, labels)
...
X["SEX"] -= 1
age_mean = X["AGE"].mean()
X["Senior"] = X["AGE"].apply(lambda x : 0 if (x <= 26) else (1 if x > 50 else 2))
...
# log_clf = LogisticRegression(C=1.9520764542603157, penalty='l1', solver='liblinear')
f1_log, acc_log, rocauc_log, eqodd_log, dempar_log = performance_report(3, log_clf, ["SEX", "Senior"])
...
# tree_clf = RandomForestClassifier(criterion='entropy', max_depth=25, max_features='sqrt', n_estimators=500)
f1_tree, acc_tree, rocauc_tree, eqodd_tree, dempar_tree = performance_report(3, tree_clf, ["SEX", "Senior"])
...
performance_report_thresh(5, thresh_log_eq, ["Senior", "SEX"])
...
performance_report_thresh(5, thresh_log_dp, ["Senior", "SEX"])
...
performance_report_thresh(5, thresh_tree_eq, ["Senior", "SEX"])
...
performance_report_thresh(5, thresh_tree_dp, ["Senior", "SEX"])
...
performance_report_reductions(1, sweep1, ["Senior", "SEX"])
...
X["default payment next month"] = y
sens_ixs = [X.columns.get_loc("Senior"), X.columns.get_loc("SEX")]
taiwan = StandardDataset(X,
                               label_name="default payment next month",
                               favorable_classes=[1],
                               protected_attribute_names=["Senior", "SEX"],
                               privileged_classes=[[1], [1]])

# taiwan.features[:,[22,1,3]]
...
roc_eq = RejectOptionClassification(unprivileged_groups=[{"Senior":0, "SEX":0}],
                                 privileged_groups=[{"Senior":1, "SEX":1}],
...
f1_roc_log_eq, acc_roc_log_eq, roc_auc_roc_log_eq, eqodd_roc_log_eq, dempar_roc_log_eq = performance_report_aif(5, taiwan, roc_eq, log_clf, sens_ixs)


# In[ ]:


roc_dp = RejectOptionClassification(unprivileged_groups=[{"Senior":0, "SEX":0}],
                                 privileged_groups=[{"Senior":1, "SEX":1}],
                                 low_class_thresh=0.1,
                                high_class_thresh=0.9,
                                  num_class_thresh=100,
...
f1_roc_log_dp, acc_roc_log_dp, roc_auc_roc_log_eq, eqodd_roc_log_dp, dempar_roc_log_dp = performance_report_aif(5, taiwan, roc_dp, log_clf, sens_ixs)


# In[ ]:


f1_roc_tree_eq, acc_roc_tree_eq, roc_auc_roc_tree_eq, eqodd_roc_tree_eq, dempar_roc_tree_eq = performance_report_aif(5, taiwan, roc_eq, tree_clf, sens_ixs)


# In[ ]:


f1_roc_tree_dp, acc_roc_tree_dp, roc_auc_roc_tree_dp, eqodd_roc_tree_dp, dempar_roc_tree_dp = performance_report_aif(5, taiwan, roc_dp, tree_clf, sens_ixs)
...
f1_platt_tree, acc_platt_tree, roc_auc_platt_tree, eqodd_platt_tree, dempar_platt_tree = performance_report_platt(5, tree_clf, ["Senior", "SEX"])


# In[ ]:


f1_platt_log, acc_platt_log, roc_auc_platt_log, eqodd_platt_log, dempar_platt_log = performance_report_platt(5, log_clf, ["Senior", "SEX"])

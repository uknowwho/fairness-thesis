...
ibm_df = pd.read_csv("HR-attrition-IBM_cleaned.csv", sep=',')
pre_value_counts = ibm_df["LABELS"].value_counts()
print(pre_value_counts)


categorical_names = ["BUSINESSTRAVEL", "DEPARTMENT", "EDUCATIONFIELD", "GENDER", "JOBROLE", "OVER18", "OVERTIME"]
categorical_ixs = [ibm_df.columns.get_loc(name) for name in categorical_names]

mappings = {}
for c in ibm_df.columns:
    if c in categorical_names:
        if not all([x.isnumeric() for x in ibm_df[c].values if type(x) == str]):
            categories = set(ibm_df[c].values)
            ...
            mappings[c] = mapping
            ibm_df[c] = ibm_df[c].map(mapping)
    ibm_df[c] = ibm_df[c].astype('float64')

ibm_df
labels = ibm_df["LABELS"]
ibm_df.drop("LABELS", inplace=True, axis=1)
...
X, y = oversample.fit_resample(ibm_df, labels)
...
X["LABELS"] = y
print(X[["GENDER", "Senior", "LABELS"]].value_counts())
...
f1_roc_log_eq, acc_roc_log_eq, roc_auc_roc_log_eq, eqodd_roc_log_eq, dempar_roc_log_eq = performance_report_aif(5, ibm, roc_eq, log_clf, sens_ixs)
...
f1_roc_log_dp, acc_roc_log_dp, roc_auc_roc_log_eq, eqodd_roc_log_dp, dempar_roc_log_dp = performance_report_aif(5, ibm, roc_dp, log_clf, sens_ixs)


# In[146]:


f1_roc_tree_eq, acc_roc_tree_eq, roc_auc_roc_tree_eq, eqodd_roc_tree_eq, dempar_roc_tree_eq = performance_report_aif(5, ibm, roc_eq, tree_clf, sens_ixs)


# In[147]:


f1_roc_tree_dp, acc_roc_tree_dp, roc_auc_roc_tree_dp, eqodd_roc_tree_dp, dempar_roc_tree_dp = performance_report_aif(5, ibm, roc_dp, tree_clf, sens_ixs)
